...
// ---------------------------------------------------------------------------
// STUB PAGES — lightweight, beginner-friendly placeholders for lessons that
// don't yet have full write-ups. Keeps every sidebar item navigable.
// ---------------------------------------------------------------------------
function stubPage(
  id: string,
  title: string,
  ...
  return {
    ...
    blocks: [
      { k: "lead", text: lead },
      { k: "h2", text: "What you'll learn" },
      { k: "ul", items: points },
      {
        k: "callout",
        tone: "note",
        text: "This lesson is being written. In the meantime, open HueSet and try the idea for yourself.",
      },
    ],
  };
}

const STUBS: [string, string, string, string, string, string[]][] = [
  ["design-principles", "Design Principles", "Compass", "Getting started",
    "The handful of ideas good designers keep coming back to — clarity, consistency, hierarchy, and respect for the person using the product.",
    ["Put the user's task first", "Keep things consistent so they feel familiar", "Guide the eye with clear hierarchy", "Design for real content, not perfect placeholder text"]],
  ["purpose", "Purpose", "Target", "Core lesson",
    "Every screen should have one clear job. Knowing that job tells you what to show, what to hide, and what to make easiest to do.",
    ["Name the main action of the screen", "Remove anything that competes with it", "Make the primary action the easiest to find", "Check the purpose is obvious without explanation"]],
  ["bd-style", "Style", "SwatchBook", "Basic design",
    "Style is the consistent look and feel that makes a product recognisable — colour, shape, type and spacing working together.",
    ["Choose a small, deliberate palette", "Reuse the same shapes and corners", "Let one style guide every screen", "Avoid mixing too many looks at once"]],
  ["bd-state", "State", "Contrast", "Basic design",
    "Most things on screen can look more than one way — resting, hovered, selected, loading, empty, or in error. Design each state, not just the happy one.",
    ["Design hover, focus and pressed states", "Plan for empty and loading views", "Show errors clearly and kindly", "Never leave a state to chance"]],
  ["bd-spacing", "Spacing", "Ruler", "Basic design",
    "Space is a design tool. Consistent gaps group related things and give the eye room to rest.",
    ["Use a small set of spacing steps", "Group related items with tighter space", "Separate sections with generous space", "Keep spacing consistent across screens"]],
  ["bd-components", "Components", "Boxes", "Basic design",
    "Components are the reusable building blocks — buttons, cards, inputs — that keep a product consistent and quick to build.",
    ["Build a piece once, reuse it everywhere", "Give each component clear rules", "Update in one place, change everywhere", "Start with the pieces you use most"]],
  ["bd-interaction", "Interaction", "MousePointerClick", "Basic design",
    "Interaction is how the product responds when someone touches it — the feedback that makes it feel alive and trustworthy.",
    ["Give quick, clear feedback to every action", "Make it obvious what can be tapped or clicked", "Confirm important actions", "Keep responses fast and predictable"]],
  ["web-users", "Users", "Users", "Website",
    "Design starts with the people who'll use it — what they need, what they know, and what gets in their way.",
    ["Know who the product is for", "Understand their goals and worries", "Use plain language they'll recognise", "Design for a range of abilities and devices"]],
  ["web-goals", "Goals", "Flag", "Website",
    "A clear goal keeps a page focused. Decide what success looks like before you design.",
    ["Define the one thing the page should achieve", "Make that outcome easy to reach", "Measure whether it's working", "Cut anything that doesn't serve the goal"]],
  ["web-usability", "Usability", "Pointer", "Website",
    "Usability is simply how easy the product is to use. If people can do what they came to do without thinking, it works.",
    ["Make actions easy to find", "Use familiar patterns", "Reduce the number of steps", "Test with real people"]],
  ["web-credibility", "Credibility", "BadgeCheck", "Website",
    "People decide quickly whether to trust a product. Clarity, polish and honesty build that trust.",
    ["Look consistent and cared-for", "Be clear about what happens next", "Show proof where it helps", "Avoid anything that feels misleading"]],
  ["web-journey", "Journey", "Route", "Website",
    "A journey is the path someone takes to reach their goal, across several screens or steps.",
    ["Map the steps from start to finish", "Smooth the handoffs between steps", "Reduce dead ends and detours", "Support going back and recovering"]],
  ["web-userflows", "User flows", "Workflow", "Website",
    "User flows show the decisions and screens between a starting point and a finished task — including the paths that go wrong.",
    ["Sketch the main path first", "Add branches for errors and choices", "Include cancel and recovery paths", "Keep each step focused"]],
  ["web-content", "Content", "FileText", "Website",
    "Content is the words, images and information people actually came for. Design should serve it, not bury it.",
    ["Lead with what matters most", "Write in plain, short language", "Break up long text", "Design around real content"]],
  ["layout-structure", "Structure", "LayoutGrid", "Layout",
    "Structure is the underlying arrangement of a page — how areas are divided so content has a logical home.",
    ["Group related content together", "Use a grid to stay aligned", "Give the page a clear top-to-bottom order", "Keep structure consistent across pages"]],
  ["layout-sections", "Sections", "Columns3", "Layout",
    "Sections break a page into digestible parts, each with a clear purpose and heading.",
    ["Give each section one idea", "Use headings to signpost", "Separate sections with space", "Order sections by importance"]],
  ["layout-headers-footers", "Headers and footers", "PanelTop", "Layout",
    "Headers and footers frame a page — navigation and identity at the top, supporting links and details at the bottom.",
    ["Keep navigation clear and consistent", "Show where the user is", "Put helpful links in the footer", "Don't overcrowd either area"]],
  ["vd-icon", "Icon", "Sparkles", "Visual design",
    "Icons are small pictures that speed up recognition — when they're clear and consistent, they help people scan.",
    ["Use a consistent icon style", "Keep meaning obvious", "Pair icons with labels when unsure", "Don't overuse them"]],
  ["vd-imagery", "Imagery", "Image", "Visual design",
    "Imagery — photos and illustrations — sets tone and mood. Choose it with intent, not as filler.",
    ["Pick images that support the message", "Keep a consistent style", "Mind quality and loading size", "Add text alternatives for accessibility"]],
  ["vd-components", "Components", "Shapes", "Visual design",
    "The visual side of components: how they look across styles and states while staying part of one family.",
    ["Keep components visually consistent", "Show every visual state", "Reuse the same styling tokens", "Let the system, not the screen, define the look"]],
  ["states-links", "Links", "Link", "States",
    "Links take people elsewhere. They should look clickable and show clear states.",
    ["Make links visibly different from text", "Show hover, focus and visited states", "Use meaningful link text", "Keep clear cues so they read as links"]],
  ["states-component", "Component", "Layers", "States",
    "Every component needs its states designed — default, hover, focus, disabled, loading and error.",
    ["List every state a component can have", "Design each one, not just the default", "Make focus visible for keyboards", "Explain disabled and error states"]],
  ["a11y-keyboard", "Keyboard navigation", "Keyboard", "Accessibility",
    "Many people navigate without a mouse. Everything should be reachable and operable with a keyboard alone.",
    ["Make every action keyboard-reachable", "Keep a logical tab order", "Avoid traps that can't be exited", "Test the whole flow with Tab and Enter"]],
  ["a11y-focus", "Focus states", "Focus", "Accessibility",
    "A visible focus indicator shows keyboard users where they are. Never hide it.",
    ["Keep a clear, visible focus ring", "Ensure it has enough contrast", "Follow a logical order", "Don't remove focus outlines"]],
  ["maint-analytics", "Analytics review", "TrendingUp", "Maintenance",
    "Analytics show how people actually use the product, so you can improve what matters.",
    ["Track key tasks and drop-off", "Look for confusing steps", "Let data question assumptions", "Review regularly, not once"]],
  ["maint-feedback", "User feedback", "MessageSquare", "Maintenance",
    "Real users tell you what analytics can't — why something feels hard.",
    ["Make it easy to give feedback", "Listen for recurring themes", "Separate symptoms from causes", "Close the loop with users"]],
  ["maint-bugs", "Bug fixes", "Bug", "Maintenance",
    "Fixing problems quickly keeps trust intact and the experience smooth.",
    ["Reproduce before fixing", "Prioritise by impact", "Prevent repeats where you can", "Verify the fix in real use"]],
  ["maint-content", "Content updates", "FilePen", "Maintenance",
    "Content ages. Keeping it accurate and clear is part of good design.",
    ["Review content on a schedule", "Remove what's outdated", "Keep language current and clear", "Check links and details still work"]],
  ["maint-designsystem", "Design system updates", "Library", "Maintenance",
    "A design system is living. Update it carefully so every product improves together.",
    ["Version changes clearly", "Communicate updates to the team", "Deprecate old pieces gracefully", "Test before rolling out"]],
  ["maint-postlaunch", "Post-launch improvements", "Rocket", "Maintenance",
    "Launch is the start, not the finish. Keep improving based on evidence.",
    ["Watch how the launch performs", "Fix the biggest pains first", "Ship small, steady improvements", "Keep measuring the impact"]],
];

for (const [id, title, icon, kicker, lead, points] of STUBS) {
  pages[id] = stubPage(id, title, icon, kicker, lead, points);
}
