...
export default function InspectorPanel(p: Props) {
  ...
  return (
    <aside className="flex h-auto w-full shrink-0 flex-col border-t border-[#e5e7eb] bg-white text-[#111827] xl:h-full xl:w-[369px] xl:border-l xl:border-t-0" aria-label="Inspector panel" style={UI_FONT}>
      <div className="flex flex-col gap-5 px-4 py-4 xl:min-h-0 xl:flex-1 xl:overflow-y-auto">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase text-[#9ca3af]">Inspector</p>
        ...
        <section aria-labelledby="actions-title">
          ...
          <button type="button" onClick={p.onRandomize} className="flex h-[46px] w-full items-center justify-center gap-2 rounded-[8px] bg-[#0e1821] px-4 text-[14px] font-semibold text-white shadow-sm hover:bg-[#1f2937] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1f9eff]">
      ...
      <div className="border-t border-[#e5e7eb] px-4 py-3 text-[11px] text-[#9ca3af]">Palette Preview</div>
    </aside>
  )
}
...
function ElementControls({ selection, values, roleOptions, onChange }: { selection: InspectorSelection; values: ElementTokenValues; roleOptions: string[]; onChange: (key: string, value: string | boolean) => void }) {
  return (
    <section aria-labelledby="element-properties-title">
      ...
      <details className="mt-5 border-t border-[#e5e7eb] pt-4 text-[12px] text-[#6b7280]">
        <summary className="cursor-pointer font-semibold text-[#4b5563]">Direct overrides</summary>
        <p className="mt-2 leading-relaxed">Use project tokens first. One-off overrides stay secondary so the design remains consistent.</p>
      </details>
    </section>
  )
}
      ...
      <div className="mt-3 grid gap-2">
        ...
        <label className="grid gap-1">
          ...
          <span className="relative">
            ...
            <select value={p.roleTargetId} onChange={(event) => p.onRoleTarget(event.target.value)} className="h-10 w-full rounded-[7px] border border-[#d7d9dd] bg-white pl-10 pr-3 text-[13px] font-semibold text-[#374151] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1f9eff]">
...
function SelectControl({ label, value, options, onChange }: { label: string; value: string; options: readonly string[]; onChange: (value: string) => void }) {
  return <label className="grid gap-1"><span className="text-[11px] font-semibold text-[#4b5563]">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} className="h-10 rounded-[7px] border border-[#d7d9dd] bg-white px-3 text-[13px] font-semibold text-[#374151] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1f9eff]">{options.map((option) => <option key={option} value={option}>{humanize(option)}</option>)}</select></label>
}
...
function ToggleControl({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex items-center justify-between rounded-[7px] border border-[#d7d9dd] px-3 py-2.5"><span className="text-[12px] font-semibold text-[#4b5563]">{label}</span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-4 w-4 accent-[#20b9fa]" /></label>
}
...
function ActionButton({ children, icon, onClick, disabled, strong }: { children: React.ReactNode; icon: React.ReactNode; onClick: () => void; disabled?: boolean; strong?: boolean }) {
  return <button type="button" onClick={onClick} disabled={disabled} className={`flex h-10 min-w-0 items-center justify-center gap-1.5 rounded-[7px] border px-2 text-[12px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1f9eff] disabled:cursor-not-allowed disabled:opacity-40 ${strong ? "border-[#d7d9dd] bg-[#f3f4f6] text-[#111827] hover:bg-[#e9eaec]" : "border-[#e5e7eb] bg-white text-[#4b5563] hover:text-[#111827]"}`}>{icon}<span className="truncate">{children}</span></button>
}

function IconButton({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} className="grid h-9 w-9 shrink-0 place-items-center rounded-[8px] border border-[#e5e7eb] text-[#6b7280] hover:text-[#111827] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1f9eff]" aria-label={label} title={label}>{children}</button>
}
