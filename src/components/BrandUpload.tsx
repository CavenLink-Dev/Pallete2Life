...
export default function BrandUpload({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/40 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="animate-pop-in max-h-[90vh] w-full max-w-lg overflow-auto rounded-2xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-lg font-bold" style={{ fontFamily: "var(--font-display)" }}>Brand assets</h2>
            <p className="mt-0.5 text-sm text-charcoal/55">See your real branding inside the Website and Mobile App previews.</p>
          </div>
          <button type="button" onClick={onClose} className="rounded-lg p-1.5 text-charcoal/50 hover:bg-offwhite" aria-label="Close">
        ...
        <p className="mt-5 text-[11px] leading-relaxed text-charcoal/45">
          Palette Preview never bundles proprietary logos, fonts or icon systems. Defaults use web-safe fonts and generic icons — upload your own supported files to see real branding.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="mt-5">
      <div className="mb-1 flex items-center justify-between">
        ...
        {value && <button type="button" onClick={() => onSet(null)} className="text-xs font-medium text-charcoal/50 hover:text-charcoal">Remove</button>}
      </div>
