...
function PaletteExport({ palette, onCopy, onText }: { palette: Swatch[]; onCopy: (value: string, label: string) => void; onText: () => void }) {
  ...
  return <Section title="Palette export" description="Copy colour values or download one readable text file."><div className="grid gap-3 sm:grid-cols-3">{formats.map(([label, value]) => <ExportCard key={label} title={label} note={value.split("\n")[0]} action="Copy" onClick={() => onCopy(value, `${label} palette`)} icon={<CopyIcon />} />)}</div><button type="button" onClick={onText} className="mt-3 flex min-h-11 w-full items-center justify-between rounded-[7px] border border-softgrey px-4 py-3 text-left hover:bg-offwhite focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-cta focus-visible:ring-offset-2"><span><span className="block text-[13px] font-bold">Palette text file</span><span className="text-[11px] text-charcoal/45">HEX, RGB and HSL together</span></span><DownloadIcon /></button></Section>
}
...
function ProjectExport({ hasSystem, accessibilityChecks, onSave, canImport, onImport }: { hasSystem: boolean; accessibilityChecks: AccessibilityCheck[]; onSave: () => void; canImport: boolean; onImport: () => void }) {
  ...
  return <Section title="Project export" description="Keep a reopenable copy of your work, including palette, tokens, template and component choices."><div className="rounded-[7px] border border-softgrey bg-offwhite p-4"><p className="text-[13px] font-bold">Project contents</p><ul className="mt-2 grid gap-1 text-[11.5px] text-charcoal/60"><li>Palette colours and locks</li><li>{hasSystem ? "Layered design tokens and component rules" : "Palette-only project settings"}</li><li>Template, brand and inspector decisions when available</li>{status && <li>Accessibility summary: {status === "good" ? ACCESSIBILITY_STATUS_LABEL.good : ACCESSIBILITY_STATUS_LABEL.review}</li>}</ul></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={onSave} className="inline-flex h-11 items-center gap-2 rounded-[7px] bg-charcoal px-4 text-[12px] font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-cta focus-visible:ring-offset-2"><DownloadIcon />Save project file</button>{canImport && <button type="button" onClick={onImport} className="inline-flex h-11 items-center gap-2 rounded-[7px] border border-softgrey px-4 text-[12px] font-bold text-charcoal hover:bg-offwhite focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-cta focus-visible:ring-offset-2"><UploadIcon />Reopen project</button>}</div></Section>
}
...
function ExportCard({ title, note, action, onClick, icon }: { title: string; note: string; action: string; onClick: () => void; icon: React.ReactNode }) {
  return <button type="button" onClick={onClick} className="flex min-h-28 flex-col rounded-[7px] border border-softgrey p-3 text-left hover:border-charcoal/25 hover:bg-offwhite focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-cta"><span className="text-charcoal/55">{icon}</span><span className="mt-3 text-[13px] font-bold">{title}</span><span className="mt-0.5 truncate text-[10.5px] text-charcoal/45">{note}</span><span className="mt-auto pt-3 text-[11px] font-bold text-brand-ink">{action}</span></button>
}
